...
public class PollVotingService {
	...
	public <T> void vote(UUID clubId, UUID pollId, T vote, Class<T> voteType) {
		...
		PollResult result = pollResultRepository.findByPollResultByPollId(pollId).orElseGet(() -> {
		...
		if (!result.getExpectedVoteType().equals(voteType)) {
			throw new IllegalArgumentException("Expected vote type: " + result.getExpectedVoteType().getSimpleName()
